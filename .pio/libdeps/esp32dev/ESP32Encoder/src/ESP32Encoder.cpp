...
ESP32Encoder::ESP32Encoder(bool always_interrupt_, enc_isr_cb_t enc_isr_cb,
    ...
    : always_interrupt{always_interrupt_}, aPinNumber{(gpio_num_t)0},
      bPinNumber{(gpio_num_t)0}, unit{(pcnt_unit_t)-1}, fullQuad{false},
      countsMode{2}, count{0}, r_enc_config{}, _enc_isr_cb(enc_isr_cb),
      _enc_isr_cb_data(enc_isr_cb_data), attached{false}, direction{false},
      working{false} {}
        ...
        if (intr_status & (BIT(i))) {
            ...
            if (PCNT.status_unit[i].COUNTER_H_LIM) {
                ...
                pcnt_counter_clear(unit);
                       ...
                       (PCNT.status_unit[i].thres0_lat ||
                        PCNT.status_unit[i].thres1_lat)) {
                int16_t c;
...
void ESP32Encoder::attach(int a, int b, enum encType et) {
    ...
    r_enc_config.pos_mode = fullQuad
                                ? PCNT_COUNT_DEC
    ...
    if (et == full) {
        ...
        r_enc_config.pulse_gpio_num =
            bPinNumber; // make prior control into signal
        r_enc_config.ctrl_gpio_num = aPinNumber; // and prior signal into
                                                 // control
        ...
        r_enc_config.lctrl_mode =
            PCNT_MODE_REVERSE;                    // prior high mode is now low
    ...
    if (!attachedInterrupt) {
        ...
        esp_err_t er = pcnt_isr_register(
            esp32encoder_pcnt_intr_handler, (void *)NULL, (int)0,
...
void ESP32Encoder::setCount(int64_t value) { count = value - getCountRaw(); }
int64_t ESP32Encoder::getCountRaw() {
    int16_t c;
    pcnt_get_counter_value(unit, &c);
    return c;
}
int64_t ESP32Encoder::getCount() { return count + getCountRaw(); }
