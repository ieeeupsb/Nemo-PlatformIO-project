...
videoCmd = "raspividyuv -w "+str(w)+" -h "+str(
    h)+" --output - --timeout 0 -vs -co 50 -br 50 --framerate "+str(fps)+" --luma --nopreview"
...
while True:
    ...
    # discard any frames that we were not able to process in time
    cameraProcess.stdout.flush()
    # Parse the raw stream into a numpy array
    frame = np.fromfile(cameraProcess.stdout,
                        count=bytesPerFrame, dtype=np.uint8)
    ...
    signed_thresh = thresh[start_height].astype(
...
def cleanup_finish():
    ...
    out = cv2.VideoWriter(
        "drive_test.avi", cv2.cv.CV_FOURCC(*"MJPG"), 5, (w, h))
